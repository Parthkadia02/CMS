...
const ViewContent = ({ blog, onClose }) => {
  const handleSendToITManager = () => {
    const storedContent = JSON.parse(localStorage.getItem("contentList")) || [];
    localStorage.setItem("contentList", JSON.stringify([...storedContent, blog]));
    alert("Sent to IT Manager successfully!");
  };
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center z-50" style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}>
      <div className="bg-white p-6 rounded-md shadow-lg w-1/2">
        <h2 className="text-xl font-bold mb-4 text-center">View Blog Details</h2>
        ...
        <div className="flex justify-end mt-4 space-x-2">
          ...
          <button onClick={handleSendToITManager} className="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600">
            Send to IT Manager
          </button>
        </div>
      </div>
